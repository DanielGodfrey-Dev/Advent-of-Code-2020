//passports must contain all the following fields, with cid as optional (sneaking in the North Pole travelers)
...
//the comma string in the data set marks the end of data for a single passport

//___________________________________________________
let passportData = [];

function generatePassports(data) {
  let passportString = '';

  //iterate through data
  for (let i = 0; i < data.length; i++) {
    //concatenate strings before each comma
    if (data[i] !== ',') {
      passportString = passportString + data[i] + ' ';
      // console.log(passportString);
    } else {
    //format into object according to passport class
      //split string into sorted array of key value pairs
    let passportStringArray = passportString.split(' ');
    passportStringArray.sort().shift();
    passportString = passportStringArray.join(' ').replace(/:/g, ' ');
    passportStringArray = passportString.split(' ');

    let passport = Passport(passportStringArray);
    checkValid(passport);

    // push to new passport object array
...
let Passport = function(data) {
  let obj = {};

  if (data.indexOf('byr') !== -1) {
    obj.byr = data[data.indexOf('byr') + 1];
  } else {
    obj.byr = null;
  }

  if (data.indexOf('iyr') !== -1) {
    obj.iyr = data[data.indexOf('iyr') + 1];
  } else {
    obj.iyr = null;
  }

  if (data.indexOf('eyr') !== -1) {
    obj.eyr = data[data.indexOf('eyr') + 1];
  } else {
    obj.eyr = null;
  }

  if (data.indexOf('hgt') !== -1) {
    obj.hgt = data[data.indexOf('hgt') + 1];
  } else {
    obj.hgt = null;
  }

  if (data.indexOf('hcl') !== -1) {
    obj.hcl = data[data.indexOf('hcl') + 1];
  } else {
    obj.hcl = null;
  }

  if (data.indexOf('ecl') !== -1) {
    obj.ecl = data[data.indexOf('ecl') + 1];
  } else {
    obj.ecl = null;
  }

  if (data.indexOf('pid') !== -1) {
    obj.pid = data[data.indexOf('pid') + 1];
  } else {
    obj.pid = null;
  }

  if (data.indexOf('cid') !== -1) {
    obj.cid = data[data.indexOf('cid') + 1];
  } else {
    obj.cid = null;
  }
...
let enhancedValidPassports = [];

function checkValidEnhanced(data) {
  for (let k = 0; k < data.length; k++) {
    ...
    for (const key in passport) {
      ...
      switch (key) {
        ...
        case 'hgt':
            let numCheck = Number((value.match(/(\d+)/)[0]));
            let endCheck = value.slice(value.length - 2, value.length);

            if ((typeof(numCheck) !== 'number')) {
              validity = false;
            }

            if ((endCheck !== 'cm') && (endCheck !== 'in')) {
              validity = false;
            }

            if (endCheck === 'cm') {
              if (!((150 <= numCheck) && (numCheck <= 193))) {
                validity = false;
              }
            }

            if (endCheck === 'in') {
              if (!((59 <= numCheck) && (numCheck <= 76))) {
                validity = false;
              }
            }
          break;
          ...
          case 'hcl':
            let chars = value.slice(1, 8);
                if ((value[0] !== '#') || (chars.length !== 6)) {
                  validity = false;
                }
              
              let numSet = Array.from(Array(10).keys());
              let charSet = ['a', 'b', 'c', 'd', 'e', 'f'];
              let compareSet = [...numSet, ...charSet];

            for (let i = 0; i < chars.length; i++) {
              let char = chars[i];
              if (char.match(/(\d+)/)) {
                char = Number(char.match(/(\d+)/)[0]);
                if (compareSet.indexOf(char) === -1) {
                  validity = false;
              } else {
                  if (compareSet.indexOf(char) === -1) {
                    validity = false;
          ...
          case 'pid':
            ...
            if ((value.match(/(\d+)/))) {
               pidCheck = Number((value.match(/(\d+)/)[0]));

              if (typeof(pidCheck) !== 'number') {
                validity = false;
              }

              let numLength = pidCheck.toString().length;
              if ((numLength < 9) && (Number(((value.slice(0, (9 - numLength))).match(/(\d+)/)[0]))) !== 0) {
                validity = false;
              }
            } else {
              validity = false;
    ...
    if (validity === true) {
      enhancedValidCount++;
      enhancedValidPassports.push(passport);
    }
  }
}
...
generatePassports(dataSet);
console.log(`We have ${passportData.length} passports...${validCount} are valid...\n`);
// console.log(`Here are the valid passports: \n`);
// console.log(validPassports);

checkValidEnhanced(validPassports);
console.log(`..under enhanced rules, we have exactly ${enhancedValidCount} valid passports.`);
console.log(`Here are the valid passports under enhanced rules: \n`);
console.log(enhancedValidPassports);