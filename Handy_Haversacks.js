function bagCount(data) {
    ...
    //establish possible bag count to increment
    let allValidBags = [];
    let newHits = true;
    //iterate through dataSet and create set of strings of bags that can directly contain shiny gold bags
    let targetBags = ['shiny gold'];
    
      while (newHits === true) {
        //...etc until iterating through original dataSet yields no additional hits
        newHits = false;
        let newTargetBags = [];
    
        for (let i = 0; i < dataSet.length; i++) {
          let rule = dataSet[i];
          let bagType = rule.slice(0, rule.indexOf('bags contain') - 1);
          let bagList = rule.slice(rule.indexOf('bags contain') + 13, rule.length);
    
          //iterate through dataSet comparing each entry to see if they contain bags from the current target bag set...
            for (let j = 0; j < targetBags.length; j++) {
              if (bagList.indexOf(targetBags[j]) !== -1) {
              //increment for positive hits
                newTargetBags.push(bagType);
              //make sure new hits aren't already in our total valid bag set
              if (allValidBags.indexOf(bagType) === -1) {
                allValidBags.push(bagType);
                newHits = true;
              }
            }
          }
    ...
    //return bag count
    console.log(allValidBags);
    return allValidBags.length;
    }
