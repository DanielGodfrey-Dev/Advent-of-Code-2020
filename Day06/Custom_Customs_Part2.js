function questions(data) {
    ...
    let groupMember = 0;
      ...
      function groupCount() {
        //define count for group
        let count = 0;
  
        //establish count for number of members in group
        let members = 0;
  
        //in counting answers EVERYONE answered yes to you in contrast to ANYONE in a group, we are shifting from boolean values to numerical
        //establish alphabet set of numerical answer count
        let answers = {
          a: 0,
          b: 0,
          c: 0,
        ...
        //establish data for each group
        while (data[groupMember]) {
  
          //iterate through group member data tallying answers in numerical tracking object
          for (let i = 0; i < data[groupMember].length; i++) {
            answers[data[groupMember][i]]++;
          }
          groupMember++;
          members++;
        }
  
        //check tallies of numerical tracking object against total number of group members
        for (let key in answers) {
          //if equal, increment total count
          if (answers[key] === members) {
            count++;
          } else {
            continue;
          }
        ...
        //add final group count to total result count
        result += count;
        groupMember++;
      }
  
      //initiate counting
      while (dataSet[groupMember]) {
        groupCount([dataSet[groupMember]]);
      }
