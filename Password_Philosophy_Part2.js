...
function dataWrangle(dataString) {
  var regex = /\d+/g;
  var matches = dataString.match(regex);  // creates array from matches
  min = parseInt(matches[0]);
  max = parseInt(matches[1]);
  letter = dataString[dataString.indexOf(':') - 1];
  password = dataString.slice(dataString.indexOf(':') + 2, dataString.length);
}

function validCheck(passwords) {
  for (let i = 0; i < passwords.length; i++) {
    dataWrangle(passwords[i]);
    if ((password[min - 1] === letter || password[max - 1] === letter) && !(password[min - 1] === letter && password[max - 1] === letter)) {positionCheck = true;}
    if (positionCheck === true) {correctPasswords++}
    positionCheck = false;
  }
}
