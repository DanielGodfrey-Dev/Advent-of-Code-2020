function bagCount(data) {
    ...
    //original outer bag type is shiny gold
    let bagType = 'shiny gold bags contain';
  
    //identify outer bag rule, then inner bags and quantity for each.
    function identifyRule (bagType) {
      //identify rule in array
      ...
      let position = searchStringInArray(bagType, data);
      let rule = data[position];
  
      // console.log(rule);
  
      //identify inner bags and quantities
      let targetString = rule.slice(rule.indexOf('bags contain') + 13, rule.length);
  
      return targetString;
    }
  
    function generateInnerBags(rule) {
      ...
      //check for multiple bag rules
      while (position !== -1) {
        indexes.push(position);
        position = rule.indexOf(',', position + 1)
      }
  
      //multiple rule case
      if (indexes.length !== 0) {
          let str = rule.slice(2, indexes[0]);
          //plurality check
          if (str[str.length - 1] !== 's') {
            str += 's';
          }
  
          let tuple = [Number(rule[0]), str + ' contain'];
          innerBags.push(tuple);
  
          for (let i = 0; i < indexes.length; i++) {
            let num = rule[indexes[i] + 2];
            let str = rule.slice(indexes[i] + 4, indexes[i + 1]);
  
            //________plurality check_______________
            if (str[str.length - 1] !== 's' && str[str.length - 1] !== '.') {
              str += 's';
            }
          ...
          let str = rule.slice(2, rule.length);
          str = str.slice(0, -1);
    ...
    //iterate over inner bags and identify multiplier and bag type (in tuples) for any given bagType
    function getTuples(bagType) {
      return generateInnerBags(identifyRule(bagType));
    }
  
    //set amount of each inner bag as multiplier, then find new rule for inner bag until inner bags contain no other bags, incrementing result count at end of tree nodes
  
    let tuples = getTuples(bagType);
    //we are descending all nodes of a tree
    function descendTree(tuples, multiplier) {
      
      function countInnerBags(array) {
        let bagCount = 0;
      ...
      for (let i = 0; i < tuples.length; i++) {
        let quantity = tuples[i][0] * multiplier;
        bagType = tuples[i][1];
        let nextBag = (getTuples(bagType));
        let innerBags = countInnerBags(nextBag);
  
        result += quantity;
        result += (quantity * innerBags);
  
        if (innerBags) {
            result -= (quantity * innerBags);
            descendTree(nextBag, quantity);
        }
      }
    }
  
    descendTree(tuples, 1);
  ...
  console.log(`Shiny gold bags contain ${bagCount(dataSet)} other bags.`);